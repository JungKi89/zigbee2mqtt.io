...
### 입력 구성
입력을 재구성해야 하는 경우(예: 순간 스위치 대신 고정형 스위치 사용 또는 그 반대), [ubisys C4에서 하는 것과 동일한 방법](C4.html#configuring-inputs)으로 수행할 수 있습니다.

### (재)바인딩 및/또는 분리
[ubisys C4 문서](C4.html#binding)도 참고하세요. 예시 사용 사례:
* 두 번째 입력으로 다른 Zigbee 디바이스 제어
* 입력을 로컬 부하에서 완전히 분리

### 안정기(Ballast) 구성
`zigbee2mqtt/FRIENDLY_NAME/set`에 발행하여 디머의 안정기 구성 속성(`min_level` 및 `max_level`)을 설정할 수 있습니다.
예시:
```json
{
    "ballast_config": {
...
`zigbee2mqtt/FRIENDLY_NAME/get/ballast_config`에 발행하여 안정기 구성 속성 값을 디바이스에서 다시 읽어 일반 Zigbee2MQTT 로그에 출력할 수도 있습니다(억제되지 않도록 경고로 표시됨).
선택적 속성이 없는 경우의 오류를 처리하기 위해(일반 함수이므로) 모든 클러스터 속성이 별도로 쿼리되므로 전체 과정에 시간이 걸릴 수 있습니다.

### 디머 전략 구성
`zigbee2mqtt/FRIENDLY_NAME/set`에 `{ "mode_phase_control": "MODE" }`를 발행하여 디머 위상 제어 모드를 변경할 수 있습니다. 여기서 `MODE`는 `automatic`(기본값), `forward`, `reverse` 중 하나입니다.

주의: 잘못된 디머 위상 제어 모드를 설정하면 디바이스나 연결된 부하가 손상될 수 있습니다!

자세한 내용은 [ubisys D1 기술 참고 매뉴얼](https://www.ubisys.de/downloads/ubisys-d1-technical-reference.pdf)의 "7.2.8. Dimmer Setup Cluster (Server)" 챕터를 참고하세요.
