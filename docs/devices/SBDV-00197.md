...
### 페어링
기기를 페어링 모드로 진입시키려면 버튼 커버를 분리하고 페어링 버튼을 5초 동안 눌러 LED가 페어링 애니메이션을 시작할 때까지 기다린 후 버튼을 놓습니다. 페어링이 완료되면 LED 애니메이션이 멈춥니다. 애니메이션 스타일은 중성선 유무에 따라 다릅니다: 중성선이 있으면 LED가 호흡하듯 켜지고, 중성선이 없으면 짧게 깜빡입니다.
