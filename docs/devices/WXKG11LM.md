...
### 동작
`triple`, `quadruple`, `hold`, `release`는 이 기기의 모든 버전에서 지원되지는 않습니다.
...
그 후 기기가 자동으로 네트워크에 참여합니다. 작동하지 않으면, 약 10초 동안 누르고 있다가 리셋 버튼을 초당 한 번씩 눌러보세요 (불빛 패턴이 버튼을 언제 뗄 수 있는지 알려줍니다).
