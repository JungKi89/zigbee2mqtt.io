...
### 조명이 꺼지지 않는 문제
조명이 전환 중(예: `{“brightness”: 20, “transition”: 5}` 전송)일 때 `{“state”: “OFF”}`로 끄지 못할 수 있습니다. 전환이 완료될 때까지 기다려야 합니다. 이는 조명 펌웨어의 [버그](https://github.com/Koenkk/zigbee2mqtt/issues/4963#issuecomment-755456697)입니다.

### 조명이 켜지지 않는 문제
밝기 1로 조명을 켤 때(예: `{“brightness” : 1}` 전송), 전구가 순간 켜졌다가 즉시 꺼집니다. 이를 해결하려면 먼저 `{“state”: “ON”}`을 전송한 후 두 번째 메시지로 밝기 `{“brightness” : 1}`을 설정하거나, 밝기를 최소 2 이상으로 제한하세요.
