...
페어링 방법은 모델마다 다를 수 있습니다(TS0043은 LoraTap SS600ZB 같은 화이트라벨 제품으로도 출시됩니다).

#### LoraTap SS600ZB:

리모컨 케이스를 엽니다(먼저 뒷면 나사를 제거합니다).
노란색 인디케이터 표시등이 깜박일 때까지 PCB의 페어링 버튼을 5~10초간 누른 후 놓습니다.
페어링하는 동안 리모컨을 코디네이터 가까이에 유지합니다.

#### Lonsonho TS0043

이 디바이스에는 PCB에 페어링 버튼이 없습니다. 왼쪽 터치 버튼을 10초간 길게 누르면 페어링 모드로 진입합니다.
