...
### 페어링
전구를 공장 초기화합니다([동영상](https://youtu.be/xeEryXGCfKA)).
초기화 후 전구가 자동으로 연결됩니다.

스위치로 다음 단계를 수행합니다:
- 전구가 꺼진 상태에서 시작
- 켜고 끄기 순서: 켜기 1초, 끄기 6초, 1초, 6초, 1초, 6초, 12초, 6초, 12초, 6초, 최종 켜기
  (켜기 지속 시간: 1초, 1초, 1초, 12초, 12초 / 끄기 간격 6초)

페어링 중에는 전구를 코디네이터(어댑터) 가까이에 두세요.
