...
## 페어링
O 버튼을 길게 눌러 Jn이 디스플레이에 표시될 때까지 누른 후 손을 떼어
연결을 시작합니다.

참고: 이미 서모스탯을 페어링했고 현재 페어링 과정 중 프리셋 모드를 변경하려면 공장 초기화를 수행해야 합니다. 그렇지 않으면 서모스탯이 이전 프리셋 모드로 페어링됩니다.

## 기기 초기화

### 소프트 리셋
O 터치 버튼을 20초 이상 길게 누릅니다.
서모스탯에 “Sr”이 깜빡여 소프트 리셋을 나타내며, 버튼을 놓으면 선택됩니다.
‘Sr’이 깜빡여 소프트 리셋을 확인합니다.

소프트 리셋을 수행하면:
- 재등록을 위해 기기가 유지하는 모든 클라우드 및 계정 정보가 삭제됩니다.
- 수동 제어의 기본 설정값으로 되돌아갑니다.
- MAC 주소 등 모든 공장 설정이 유지됩니다.
- 재연결/재등록 이전 및 이후 정상 작동을 보장하기 위한 설치 구성이 유지됩니다.

### 공장 초기화
리셋 선택 중, O 터치 버튼을 서모스탯에 “Fr”이 깜빡여 공장 초기화를 나타낼 때까지 길게 누르고, 버튼을 놓으면 선택됩니다.
매트릭스 디스플레이에 ‘Fr’이 깜빡여 공장 초기화를 확인하며, 서모스탯은 공장 기본값으로 재설정되고 5초 후 프리셋으로 돌아갑니다.
...
## 프로그램 모드 프리셋
모드 P1-P6은 실내 제어 모드 4...30°C
모드 P7-P8은 바닥 제어 모드 10...40°C

P1 - 히트 펌프/오일 보일러
P2 - 수압식 라디에이터/가스 보일러
P3 - 수압식 바닥 난방
P4 - 전기 라디에이터
P5 - 전기 바닥 난방 (바닥 온도 제한 없음)
P6 - 전기 바닥 난방 (바닥 온도 제한 있음)
P7 - 수압식 바닥 난방
P8 - 전기 바닥 난방
P9 - 레귤레이터 모드 (출력이 퍼센트로 표시됨)
