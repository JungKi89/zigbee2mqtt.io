...
### 화이트 채널
화이트 채널을 활성화하려면 `zigbee2mqtt/DEVICE_FRIENDLY_NAME/set`에 다음 페이로드 중 하나를 전송합니다:

1. `{"color":{"rgb":"255,255,255"}}`
1. `{"color":{"hex":"#FFFFFF"}}`
1. `{"color":{"x":0.3125, "y": 0.32894736842105265}}` - 흰색을 선택할 때 Zigbee2MQTT 프론트엔드가 전송하는 x/y 값
1. `{"color":{"x":0.323, "y": 0.329}}` - 흰색을 선택할 때 Home Assistant가 전송하는 x/y 값 (컬러 피커 휠 중앙)
