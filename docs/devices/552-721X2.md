...
### 페어링
커버를 제거하고 작은 버튼 중 하나를 3번 누르세요. 페어링 모드에서 LED가 파란색으로 깜박입니다. 기기는 5분 동안 또는 성공적으로 페어링될 때까지 이 모드를 유지합니다.

### 바인딩
이 기기는 그룹 또는 기기에 대한 바인딩을 지원하지 않습니다.

### 분리(Decoupling)
이 기기는 내장 릴레이에서 분리를 지원하며, 분리된 경우 'single', 'hold', 'release' 액션을 발생시킵니다.
...
| 꺼짐 | 정상 동작, 부하 꺼짐 |
| 흰색 연속 | 정상 동작, 부하 켜짐 |
| 빨간색 깜박임 | Zigbee 네트워크 연결 끊김 |
| 파란색 깜박임 | Zigbee 네트워크 연결 중 |
| 파란색 4초 연속 | Zigbee 네트워크 연결 성공 |

부하가 켜진 상태에서 이쑤시개로 **PROG** 버튼을 10초간 누르면 LED를 비활성화할 수 있습니다.
