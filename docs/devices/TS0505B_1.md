...
처음 전원을 켜면 조명이 페어링 모드에 있습니다.

Tuya 조명을 초기화하려면:
먼저 디바이스 전원을 켜기 전에 10초 이상 전원이 꺼져 있었는지 확인합니다.
그런 다음 디바이스 전원을 세 번 켜고 끕니다(on-off-on-off-on, 각 켜기와 끄기 사이에 2~4초 간격).
조명이 빠르게 깜박이기 시작할 때까지 잠시 기다립니다. 디바이스가 페어링 준비 상태입니다
