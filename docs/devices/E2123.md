...
### 펌웨어 1.0.32 (20221219) 참고사항

* `1.0.012 (20211214)`에서 리모컨을 업데이트한 후 기기를 재설정하거나 다시 페어링해야 할 수 있습니다.
* 닷 한 번 누르기는 여러 액션을 트리거합니다:
  * `dot_<1|2>_initial_press`
  * `dot_<1|2>_short_release`
* 닷 두 번 누르기는 여러 액션을 트리거합니다:
  * `dot_<1|2>_initial_press`
  * `dot_<1|2>_double_press`
* 길게 누르기(누름 + 떼기)는 여러 액션을 트리거합니다:
  * `dot_<1|2>_initial_press`
  * `dot_<1|2>_long_press`
  * `dot_<1|2>_long_release`

### 펌웨어 1.0.35 (20230308) 참고사항

* 닷 한 번 누르기, 닷 두 번 누르기, 길게 누르기는 1.0.35와 동일하게 여러 액션을 트리거합니다.
* 업데이트 후 닷 버튼이 작동하지 않으면 기기를 제거하고 다시 페어링해야 합니다.
