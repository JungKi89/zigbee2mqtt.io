...
엔드포인트 `1`이 조명 또는 조명 그룹에 바인딩되면 동작은 다음과 같습니다 (단일 밴드 모델):
| 버튼 | 클릭 | 동작 | 설명 |
|-----|-----|-----|-----|
| 왼쪽 | 단일 클릭 | 끄기 | `commandOff` 명령 사용 |
| 오른쪽 | 단일 클릭 | 켜기 | `commandOn` 명령 사용 |
| 왼쪽 | 더블 클릭 | 밝기 감소 | 85포인트(33%) 단위로, `step` 명령 사용 |
| 오른쪽 | 더블 클릭 | 밝기 증가 | 85포인트(33%) 단위로, `step` 명령 사용 |
| 왼쪽 | 길게 누름 | 색온도 감소 | 69 mired 단위로 `stepColorTemp` 명령 사용 (더 따뜻하게) |
| 오른쪽 | 길게 누름 | 색온도 증가 | 69 mired 단위로 `stepColorTemp` 명령 사용 (더 차갑게) |
