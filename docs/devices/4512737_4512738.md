...
## Notes
### Home Assistant UI에서 바닥 센서 사용하기

기본적으로 `local_temperature` (디바이스에서 측정된 공기 온도)가 HA의 온도 조절기 엔티티의 현재 온도로 사용됩니다. `outdoor_temperature`를 통해 노출되는 바닥 센서를 사용하려면 다음과 같이 설정하십시오:

1. Zigbee2MQTT의 `configuration.yaml`에서 기후 디바이스 아래에 `current_temperature_template` 줄을 추가합니다:
...
1. 변경 사항을 적용하려면 Zigbee2MQTT를 재시작합니다.

`current_temperature_template`에 대한 문서는 여기에서 확인할 수 있습니다: https://www.home-assistant.io/integrations/climate.mqtt/#current_temperature_template
