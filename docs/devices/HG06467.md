...
### 페어링
"F" 버튼을 5초간 누르고 있으면 LED 스트링이 공장 초기화됩니다.
버튼을 놓으면 LED 스트링이 천천히 깜박입니다.
초기화 후 LED 스트링이 자동으로 연결됩니다.

페어링 중에는 스트링을 어댑터 가까이에 두세요.


### 효과 트리거
LED 스트링의 내장된 16가지 효과를 제어합니다. 효과에는 `speed`, `colors`, `effect` 3개의 매개변수가 필요합니다. 효과를 실행하려면 `zigbee2mqtt/FRIENDLY_NAME/set`로 `{"effect": {"effect": EFFECT, "speed": SPEED, "colors": COLORS}}` 페이로드와 함께 메시지를 보냅니다. 설명:
