...
## Notes
이 디바이스는 채널 15보다 높은 Zigbee 채널을 지원하지 않는 것으로 보입니다. 네트워크가 더 높은 Zigbee 채널을 사용하는 경우, 이 디바이스를 네트워크에 참가시키지 못할 수 있습니다.

### 페어링

* ELKO Super TR RF 디스플레이에 "joining"과 "pairing"이 표시될 때까지 "arrow up"과 "M" 버튼을 동시에 누릅니다.
