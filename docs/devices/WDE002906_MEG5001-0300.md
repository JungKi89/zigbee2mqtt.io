...
### 페어링
페어링 모드에 진입하려면 오른쪽 상단 버튼을 3번 빠르게 누릅니다. LED가 주황색으로 깜빡입니다. 연결이 완료되면 LED가 초록색으로 바뀝니다.

### 공장 초기화
기기를 초기화하려면 오른쪽 상단 버튼을 0.5초 이내에 3번 빠르게 누른 후 최소 10초간 길게 누릅니다. LED가 빨간색으로 깜빡이기 시작하면 버튼을 놓습니다. 초기화에 성공하면 LED가 초록색으로 몇 번 깜빡입니다.
Bluetooth에서 Zigbee로 연결 모드를 변경하려면 10초 대신 20초간 누릅니다 (빨간 LED가 더 빠르게 깜빡임).

### 모드 변경
1구에서 2구로 변경할 수 있습니다.
왼쪽 상단과 오른쪽 하단 버튼을 10초간 누릅니다 (왼쪽과 오른쪽의 LED 2개가 깜빡이기 시작함).

1구로 변경하려면 공장 초기화가 필요합니다.

### 배터리
기기는 CR2032 배터리 1개를 사용합니다.
