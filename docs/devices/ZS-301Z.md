...
이 기기의 일부 버전(예: "C3007" 모델, 보드에 "ZSSF01"이라고 인쇄된 것)은 보고 간격을 설정할 수 없는 상태에서 지나치게 많은 메시지를 발생시킵니다. 이로 인해 배터리가 매우 빠르게 소모되며(수일 내), 설정값이 유지되지 않는 등의 다른 문제도 발생합니다. 자세한 내용은 https://github.com/Koenkk/zigbee2mqtt/issues/29254 를 참조하세요.
