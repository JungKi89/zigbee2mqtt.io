...
## Notes
이 디바이스에서 OTA 업데이트가 작동하려면 업데이트가 완료될 때까지 디바이스의 버튼을 계속 반복해서 눌러야 zigbee2mqtt와의 연결이 유지됩니다.
중요: 이 디바이스에서 사용 가능한 최신 펌웨어는 디바이스를 완전히 사용 불가능하게 만듭니다. **업데이트를 적용하지 마세요!** 자세한 내용은 이슈를 참조하세요: https://github.com/Koenkk/zigbee2mqtt/issues/25207

### 페어링
디바이스의 리셋 버튼을 약 5초간(디스플레이 상단의 네트워크 표시등이 깜박이기 시작할 때까지) 누르고 있습니다.
이후 디바이스가 자동으로 참가합니다.
