...
### 페어링
기기를 페어링 모드로 진입시키려면 버튼을 10초 동안 눌러 LED가 호흡 애니메이션을 시작할 때까지 기다린 후 버튼을 놓습니다. 페어링이 완료되면 LED 애니메이션이 멈춥니다.
...
### 비상 모드
이 기기에는 내부 보호 로직이 있습니다: 모니터링되는 파라미터(전압, 전류 또는 온도) 중 하나라도 한계를 벗어나면 기기가 꺼집니다(비상 모드에 진입). 전압이 너무 높거나 낮아서 비상 모드에 진입한 경우 전압이 정상 범위로 돌아오면 이전 상태로 복귀합니다. 과열 또는 과전류로 인해 비상 모드에 진입한 경우 수동으로 다시 켜기 전까지 꺼짐 상태를 유지합니다. 내부적으로 주기적인 측정이 수행되며, 외부 보정 옵션(*_calibration)은 내부 보호 로직에 적용되지 않습니다.
