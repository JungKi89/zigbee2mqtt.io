...
## Notes
### 페어링
이 기기의 페어링을 시작하려면, 메뉴가 진입될 때까지(디스플레이가 PX로 변경되고 X가 깜빡이는 숫자) 메인 노브를 길게 누른 뒤 노브를 짧게 눌러 P0을 선택합니다. 그러면 디스플레이에 CF와 깜빡이는 안테나 심볼이 표시되고, 완료되면 실제 온도와 안정적인 안테나 심볼로 전환됩니다.

### 펌웨어
펌웨어 버전이 >=0.2.3이어야 합니다. 이전 펌웨어 버전은 정상적으로 작동하지 않습니다. 제조사 앱을 통해 업데이트하세요 (Tuya 호환 게이트웨이/허브 필요).
