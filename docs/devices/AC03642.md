...
### 페어링
[Manual reset](https://www.belkin.com/support-article/?articleNum=159344)의 지침을 따르세요.
초기화 후 전구는 자동으로 연결됩니다.


### 기본 전원 켜기/끄기 전환 설정
다양한 Osram/Sylvania LED는 조명을 켜고 끌 때 기본 전환 설정을 지원합니다.
...
```
**INFO**: 값은 초 단위 시간입니다(정수, 부동 소수점 값은 지원되지 않음)

### 현재 조명 상태 기억
다양한 Osram/Sylvania LED는 정전이나 조명이 수동으로 껐다가 켜지는 경우 현재 상태를 기억하는 것을 지원합니다. 조명은 각각의 속성(밝기, 색상, 채도 등)을 기억합니다.
**참고**: 조명 속성을 변경할 때마다 이 명령을 실행해야 해당 상태가 '기억'됩니다.
...
```
**INFO**: 값은 true, false(불리언)

**참고**: 전구에 펌웨어 버그가 있습니다. 예를 들어 밝기 값을 105로 설정하고 기억한 후 일반 스위치로 껐다 켜면, 전구가 저장했을 때보다 더 밝게 켜집니다. 그러나 `{"brightness": ""}`로 조회하면 전구는 밝기 105를 보고합니다. `{"state": "off"}` 및 `{"state": "on"}`을 전송하면 저장했을 때와 동일한 밝기로 켜지지만, 여전히 이 예시에서 밝기 105를 보고합니다.
