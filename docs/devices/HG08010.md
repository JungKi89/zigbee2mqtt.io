...
### 페어링
설명서(IAN 378599 QSG_DE, 설명서에 사용된 Lidl 내부 번호)에 설명된 페어링 방법은 작동하지 않습니다!
Lidl HG08010(IAN 378599 OS_DE)으로 인식되는 다른 기기 유형은 직접 소유하고 있지 않아 확인할 수 없습니다. 하지만 동일한 절차일 것으로 생각됩니다.

기기를 초기화하고 페어링을 시작하려면 스위치로 다음 순서에 따라 조명을 끄고 켭니다:
1. 조명이 켜진 상태에서 시작합니다.
2. 조명 끄기 → 10초 대기.
3. 조명 켜기 → 1초 대기.
4. 조명 끄기 → 3초 대기.
