...
### 페어링
기기를 페어링 모드로 진입시키려면 버튼 커버를 분리하고 페어링 버튼을 5초 동안 눌러 LED가 호흡 애니메이션을 시작할 때까지 기다린 후 버튼을 놓습니다. 페어링이 완료되면 LED 애니메이션이 멈춥니다.

### 공장 초기화
이 기기는 Zigbee 네트워크에서 제거될 때 공장 초기화됩니다. 단, 코디네이터에 의해 시작된 경우(즉, 프론트엔드를 통해 또는 `zigbee2mqtt/bridge/request/device/remove` 토픽으로 명령을 전송하는 경우)에만 해당됩니다. 페어링 버튼을 눌러 수동으로 기기를 제거한 경우 설정은 그대로 유지됩니다.
