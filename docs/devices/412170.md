...
공장 기본값인 device_mode는 Legrand 배선 지침에 나와 있는 L에서 C1 푸시버튼 배선 방식에서 불규칙한 릴레이 동작을 유발합니다. 예측 가능한 동작을 위해 device_mode를 switch로 설정하세요. auto 값은 412171 접촉기 항목을 복사하면서 남겨진 것으로 보입니다: 이 412170에는 공식 Legrand Home+Control 앱에서 해당하는 "Off-peak/peak time" (auto) 설정이 없습니다 (412171과 달리).
