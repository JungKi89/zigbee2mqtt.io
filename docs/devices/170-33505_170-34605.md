...
### 페어링
소켓 아울렛을 네트워크에 페어링하려면 6초 이내에 버튼을 최소 3번 누릅니다. 그러면 소켓 아울렛이 5분간 페어링 모드로 유지됩니다. LED가 파란색으로 깜빡입니다.

### 공장 초기화
연결된 소켓 아울렛을 공장 설정으로 초기화할 수 있습니다.
A. 버튼을 약 1초간 누른 채로 유지합니다.
B. LED가 파란색으로 바뀌면 놓고, 다시 버튼을 누른 채로 유지합니다.
C. LED가 깜빡이기 시작하면 10번 깜빡인 후 놓습니다.
D. LED가 흰색으로 바뀔 때까지 기다립니다.

### 어린이 잠금
어린이 잠금이 활성화되면 물리적 버튼뿐만 아니라 MQTT를 통해서도 장치 상태를 변경할 수 없습니다!
