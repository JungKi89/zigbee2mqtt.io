...
## Notes
전원 켜짐 동작의 'Off' 옵션은 처음 4개 채널(1-4)에서 작동하지 않습니다. 전원이 복구된 후, 전원 차단 중에 처음 4개 릴레이 중 하나라도 켜져 있었다면 기기의 LED가 꺼져 있고 Zigbee2MQTT에서도 꺼진 것으로 표시되더라도 해당 릴레이는 'on' 상태로 유지됩니다.
