...
### 페어링
LEDVANCE Smart+ AC25697 전구를 초기화하려면 조명이 켜진 상태에서 시작하여, 5초 끄기와 5초 켜기를 5번 반복합니다:
켜기, 5초 끄기, 5초 켜기, 5초 끄기, 5초 켜기, 5초 끄기, 5초 켜기, 5초 끄기, 5초 켜기, 5초 끄기, 켜기. 이 과정 후 조명이 번갈아 색상을 바꾸며 깜박여야 합니다.

### 밝기 초기화
전원을 끈 후 밝기가 `1/255`로 초기화되는 문제를 겪는 사용자가 있습니다. 전구를 다시 페어링하면 이 문제가 해결되는 것으로 보입니다.

### 기본 켜기/끄기 전환 설정
다양한 Osram/Sylvania LED는 조명을 켜고 끌 때 기본 전환 효과를 설정할 수 있습니다.
...
### 현재 조명 상태 기억
다양한 Osram/Sylvania LED는 전원이 꺼지거나 조명이 수동으로 껐다가 켜진 경우 현재 상태를 기억할 수 있습니다. 조명은 각각의 속성(밝기, 색상, 채도 등)을 기억합니다.
**NOTE**: 조명 속성을 변경할 때마다 이 명령을 실행해야 변경 사항이 '기억'됩니다.
