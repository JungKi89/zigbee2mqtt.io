...
### 시계 설정
온도 조절기 화면 시계를 설정하려면 다음 MQTT 토픽으로 epoch 값을 전송해야 합니다:
```
zigbee2mqtt/<FRIENDLY_NAME>/set/thermostat_time
```

기기 페어링 시 시간이 네트워크 시간으로 설정됩니다. 이후 온도 조절기는 DST와 같은 네트워크 시간 변경에 자동으로 업데이트되지 않습니다.

빈 페이로드를 전송하면 현재 네트워크 시간으로 설정됩니다.

Home Assistant를 사용하여 시계 설정을 자동화하려면 다음과 같은 자동화를 만들 수 있습니다:
...
- **ecoMode** : off=-128, 범위: -100~100, 피크 및 예열 시 설정값 감소/증가량, 0은 에코 아이콘만 깜빡임
- **ecoMode1** : off=255, 범위: 0~99, pi_heating_demand 백분율 한계 0%~99% 설정 (255 = 100%)
...
스크립트(이 예시에서는 python)를 사용하여 에코 모드를 활성화하려면 다음과 같이 형식화된 MQTT 메시지를 전송해야 합니다:
``` python
topic = "zigbee2mqtt/<FRIENDLY_NAME>/1/set"
payload = {"write": {"cluster": "manuSpecificSinope", "options": {}, "payload": {"ecoMode": <VALUE>}}}
```
