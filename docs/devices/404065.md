...
이 디바이스(펌웨어 버전 `MDFIL201`)는 펌웨어 문제가 [알려져](https://github.com/Koenkk/zigbee2mqtt/issues/15832) 있습니다. 요청된 밝기가 `103` 미만일 경우 트랜지션을 사용하면 전구가 켜지지 않고, 잠깐 깜박인 후 꺼지며 `off` 상태를 보고합니다.
