...
GU10 스팟인 모델 45327(펌웨어 버전 `MGU10501`)과 E14 전구인 모델 45328(펌웨어 버전 `MC37501`)은 펌웨어 문제가 [알려져](https://github.com/Koenkk/zigbee2mqtt/issues/15832) 있습니다. 요청된 밝기가 `103` 미만일 경우 트랜지션을 사용하면 전구가 켜지지 않고, 잠깐 깜박인 후 꺼지며 `off` 상태를 보고합니다.

### 페어링
새 전구는 자동으로 페어링 모드가 활성화되어 있습니다.
전구를 1초 간격으로 다섯 번 껐다 켜면 공장 초기화할 수 있습니다. 공장 초기화가 성공하면 색상 전환 효과가 나타납니다.
