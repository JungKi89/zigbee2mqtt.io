...
## 난방 또는 냉방
이 서모스탯은 난방 또는 냉방 모드로 작동할 수 있습니다. `control_sequence_of_operation`을 'heating_only'(초기화 후 기본값) 또는 'cooling_only'로 설정하여 모드를 전환할 수 있습니다.

노출된 데이터는 동적으로 적응하지만, `control_sequence_of_operation`을 변경한 후에는 Zigbee2MQTT를 재시작해야 업데이트됩니다.
