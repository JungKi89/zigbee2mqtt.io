...
### 기본 전원 켜기/끄기 전환 설정
다양한 Osram/Sylvania LED는 조명을 켜고 끌 때 기본 전환(transition) 값을 설정하는 것을 지원합니다.
...
```
**INFO**: 값은 초 단위 시간입니다 (정수, 소수점 값은 지원하지 않음)

### 현재 조명 상태 기억
다양한 Osram/Sylvania LED는 전원이 나가거나 조명을 수동으로 껐다가 켤 경우를 대비해 현재 상태를 기억하는 기능을 지원합니다. 조명은 밝기, 색상, 채도 등 각각의 속성을 기억합니다.
**NOTE**: 조명 속성을 변경할 때마다 이 명령을 실행해야 해당 상태가 기억됩니다.
