...
### 하단 4번째 스위치에 대해
이 버튼은 마스터 스위치로 작동합니다 -- 나머지 세 스위치를 모두 켜고 끕니다.
이 버튼은 엔드포인트를 노출하지 않습니다.
이 버튼을 2초 동안 길게 누르면 --> 페어링 모드 (모든 조명이 2번 깜박이며, 조명이 켜진 채로 유지되면 페어링 성공)
이 버튼을 10초 동안 길게 누르면 --> 언페어링 모드, 모든 조명이 4번 깜박입니다.
