...
### 입력 설정
입력을 재설정해야 하는 경우(예: 모멘터리 스위치 대신 고정식 스위치 사용 또는 그 반대), [ubisys C4에 대해 수행하는 방법](C4.html#configuring-inputs)과 동일한 방법으로 할 수 있습니다.

### (재)바인딩 및/또는 분리
[ubisys C4 문서](C4.html#binding)도 참고하세요. 사용 사례 예시:
* 두 번째 입력을 사용하여 다른 Zigbee 기기 제어 (S1-R만 해당, S1은 입력이 하나뿐)
* 입력을 로컬 부하에서 완전히 분리

분리된 경우 연결된 스위치 버튼을 누를 때 'action'이 게시됩니다.

### 기술 참조
자세한 내용은 [ubisys S2 기술 참조 매뉴얼](https://www.ubisys.de/downloads/ubisys-s2-technical-reference.pdf)을 참고하세요.
