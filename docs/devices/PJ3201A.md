...
재실(Occupancy)은 활발한 움직임을 나타냅니다. 이 속성은 움직임 지연의 직접적인 영향을 받지 않습니다. 단, 움직임만 감지되고 재실(presence)이 감지되지 않은 경우, 움직임 지연은 재실 상태가 초기화되는 시간에 영향을 줍니다.

공장 초기화를 임의의 상태로 전환하면 공장 설정이 복원됩니다. 이 작업은 Zigbee 설정을 초기화하지 않으며, 센서 기본값만 불러옵니다.
