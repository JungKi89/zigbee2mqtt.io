...
### 페어링
* 배터리를 삽입합니다. "o" 버튼을 3초간 눌러 M 표시가 나타날 때까지 기다립니다. 밸브에 장치를 설치합니다.
* "o" 버튼을 짧게 눌러 페어링 모드로 진입합니다.
* Zigbee2MQTT에서 장치를 추가합니다.
* "o" 버튼을 3초간 길게 누릅니다. 이제 장치가 설치되었으며 현재 온도가 표시됩니다.

공장 초기화 방법:
1. Ally™ 라디에이터 온도조절기의 커버를 제거합니다.
2. 두 개의 배터리 중 하나를 제거합니다.
3. 배터리를 교체하는 동안 디지털 디스플레이/화면 옆의 검은 버튼을 누른 채로 유지합니다.
4. 배터리를 삽입하고 10초 후에 버튼을 놓습니다.
5. Ally™ 라디에이터 온도조절기가 초기화되었습니다.
6. Ally™ 라디에이터 온도조절기의 커버를 다시 장착합니다.

### OTA 업데이트 후 버전 번호 문제

OTA를 사용하여 펌웨어를 업데이트하면 `AAAABBBBCCCCDDDD`와 같은 버전 번호가 표시될 수 있습니다. 이 문제를 해결하려면 장치를 다시 인터뷰하면 됩니다(장치 페이지의 파란색 `i` 아이콘을 클릭).
