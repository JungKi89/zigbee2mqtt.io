...
### PIN 코드 사용법
상태를 조회하려면 장치 토픽(`zigbee2mqtt/DEVICE_FRIENDLY_NAME/get`)에 `{"pin_code":{"user":0}}` 본문으로 `get` 메시지를 전송합니다. 설정하려면 장치 토픽(`zigbee2mqtt/DEVICE_FRIENDLY_NAME/set`)에 `{"pin_code":{"user":0,"pin_code":1234}}` 본문으로 `set` 메시지를 전송합니다. 코드를 지우려면 `set`을 호출하되 `pin_code` 값을 생략합니다.
