...
### 실외 온도 설정
*실외 온도*를 설정하려면 다음 MQTT 토픽에 값을 전송해야 합니다:
```
zigbee2mqtt/<FRIENDLY_NAME>/set/thermostat_outdoor_temperature
```

Home Assistant를 사용하여 실외 온도 게시를 자동화하려면 다음과 같은 자동화를 만들 수 있습니다:
...
**중요**: 실외 온도는 최소 4시간마다 갱신해야 합니다. 그렇지 않으면 서모스탯의 `EXT` 표시가 지워집니다.

### 에너지 값 초기화
최대 에너지 값은 `500`이며 직접 쓰기가 허용되지 않아 수동으로 초기화할 방법이 없습니다. 이 값은 서모스탯이 자정에 자동으로 초기화합니다. 서모스탯이 자정에 초기화를 수행할 수 있도록 시간을 설정하려면, Hilo 게이트웨이가 매분마다 다음과 같은 브로드캐스트를 전송하는 동작을 재현해야 합니다:
```
...
이 스크립트를 매분 실행할 필요는 없을 수 있지만, 자정 전후 몇 분 동안 여러 번 실행하면 서모스탯에 올바른 시간이 설정되어 날짜 변경을 인식하도록 하는 데 도움이 될 수 있습니다.
