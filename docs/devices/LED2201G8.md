...
### 페어링
6번 켜기/끄기 방법으로 전구를 공장 초기화합니다 ([동영상](https://www.youtube.com/watch?v=npxOrPxVfe0)).

초기화 후 전구가 자동으로 연결됩니다.

페어링 중에는 전구를 코디네이터(어댑터) 가까이 두세요.

효과적인 방법은 전구가 켜지려는 기미가 보이면 바로 끄는 방식으로, (매우) 짧게 켰다가 조금 더 길게 끄기를 반복하는 것입니다.
전구를 켠 상태에서 시작하여 끄고, 이후 6번 켜기를 반복한 뒤 6번째 ON 상태에서 기다립니다. (안전하게 7번 켜기를 시도하면 초기화가 실패하는 경우가 있습니다.)

터치링크 공장 초기화는 현재(펌웨어 2.4.5 기준) 이 전구에서 작동하지 않는 것으로 보입니다.
