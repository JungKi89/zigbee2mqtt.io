...
### 실외 온도 설정
*실외 온도*(온도 조절기 화면의 Out 라벨 아래 값)를 설정하려면 다음 MQTT 토픽으로 값을 전송해야 합니다:
...
### 시간 활성화
시간을 활성화하려면 다음 MQTT 토픽으로 빈 메시지를 전송해야 합니다:
...
온도 조절기의 전원이 끊기면 이 작업을 다시 해야 합니다. Home Assistant 사용자의 경우: 이를 기억해야 하는 번거로움을 피하려면 위에서 언급한 `Auto_Publish_Outdoor_Temperature` 자동화에 다음 `action`을 추가할 수 있습니다.
