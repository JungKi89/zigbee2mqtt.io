...
엔드포인트 `1`이 조명 또는 조명 그룹에 바인딩되면 동작은 다음과 같습니다 (트리플 밴드 모델):
| 버튼 | 클릭 | 동작 | 설명 |
|-----|-----|-----|-----|
| 상단, 왼쪽 | 단일/더블/길게 | 끄기 | `commandOff` 명령 사용 |
| 상단, 오른쪽 | 단일/더블/길게 | 켜기 | `commandOn` 명령 사용 |
| 가운데, 왼쪽 | 단일/더블 | 밝기 감소 | 85포인트(33%) 단위로, `step` 명령 사용 |
| 가운데, 오른쪽 | 단일/더블 | 밝기 증가 | 85포인트(33%) 단위로, `step` 명령 사용 |
| 가운데, 왼쪽 | 길게 누름 | 밝기 서서히 감소 | 15포인트(약 6%) 단위로, `commandMove` 명령 사용 (버튼 놓으면 `commandStop` 명령 전송) |
| 가운데, 오른쪽 | 길게 누름 | 밝기 서서히 증가 | 15포인트(약 6%) 단위로, `commandMove` 명령 사용 (버튼 놓으면 `commandStop` 명령 전송) |
| 하단, 왼쪽 | 단일/더블 | 색온도 감소 | 69 mired 단위로 `stepColorTemp` 명령 사용 (더 따뜻하게) |
| 하단, 오른쪽 | 단일/더블 | 색온도 증가 | 69 mired 단위로 `stepColorTemp` 명령 사용 (더 차갑게) |
| 하단, 왼쪽 | 더블 클릭 | 색온도 서서히 감소 | 15 mired 단위로 `commandMoveColorTemp` 명령 사용 (더 따뜻하게) |
| 하단, 오른쪽 | 더블 클릭 | 색온도 서서히 증가 | 15 mired 단위로 `commandMoveColorTemp` 명령 사용 (더 차갑게) |
