...
- [CSA IoT - Zigbee 인증](https://csa-iot.org/csa_product/lidl-home-smart-colour-change-led-light-bulb-3/)

### 페어링
전구를 공장 초기화합니다.
초기화 후 전구가 자동으로 연결됩니다.

사용할 수 있는 방법 중 하나는 전구가 켜지려는 기미가 보이는 순간 전원을 차단하는 방식으로, 매우 짧게 “켜기”를 하고 조금 더 길게 “끄기”를 반복하는 것입니다.
전구가 켜진 상태에서 시작하여 끄고, 이후 3번 “켜기”를 하고 3번째 ON 상태에서 기다립니다.
또는 전구가 켜진 상태에서 위와 동일한 끄기/켜기 순서를 반복하되 각 상태(켜기 및 끄기)에서 2~3초씩 기다립니다.
페어링 상태에서는 전구가 깜박입니다.
