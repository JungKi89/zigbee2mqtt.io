...
페어링 버튼은 기기 전면 패널 아래에 있습니다.

1. 상단 두 클립을 위로 들어 올려 전면 패널을 제거하세요.
2. 전면 패널 뒤의 제어 모듈에 있는 작은 페어링/리셋 버튼을 찾으세요.
3. LED/팬 인디케이터가 변경되기 시작할 때까지(밝아졌다 어두워짐) 페어링 버튼을 약 3초간 누르고 있으세요. 이는 페어링 모드 진입을 나타냅니다.
